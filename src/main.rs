...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if is_new_file {
        writeln!(writer, "timestamp,manufacturer,model,battery").expect("Failed to write header");
    }
        ...
        if peripherals.is_empty() {
            ...
            for peripheral in peripherals.iter() {
                ...
                if is_connected {
                    ...
                    let device_info_serv = services
                        .iter()
                        .find(|c| c.uuid == DEVICE_INFORMATION_UUID)
                        .expect("Unable to find device information service");
                    let model_char = device_info_serv
                        .characteristics
                        .iter()
                        .find(|c| c.uuid == MODEL_NUMBER_CHARACTERISTIC_UUID)
                        .expect("Unable to find model number characteristic");
                    let manufacturer_char = device_info_serv
                        .characteristics
                        .iter()
                        .find(|c| c.uuid == MANUFACTURER_CHARACTERISTIC_UUID)
                        .expect("Unable to find manufacturer characteristic");

                    let batt_serv = services
                        .iter()
                        .find(|c| c.uuid == BATTERY_SERVICE_UUID)
                        .expect("Unable to find battery service");
                    let batt_char = batt_serv
                        .characteristics
                        .iter()
                        .find(|c| c.uuid == BATTERY_LEVEL_CHARACTERISTIC_UUID)
                        .expect("Unable to find battery level characteristic");
                    debug!("  {:?}", batt_char);
                    let batt_lvl_value = peripheral.read(batt_char).await?;
                    info!("{:} Battery value: {:?}", local_name, batt_lvl_value);

                    let device_status = DeviceStatus {
                        manufacturer: String::from_utf8(peripheral.read(manufacturer_char).await?)
                            .unwrap_or_else(|e| {
                                format!("Invalid UTF-8 sequence for manufacturer: {}", e)
                            }),
                        model: String::from_utf8(peripheral.read(model_char).await?)
                            .unwrap_or_else(|e| format!("Invalid UTF-8 sequence for model: {}", e)),
